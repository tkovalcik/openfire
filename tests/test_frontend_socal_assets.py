...
def test_socal_deckgl_prototype_is_isolated_static_variant() -> None:
    ...
    # The heatmap refactor needs the prediction GeoJSON reachable via the
    # /data/ proxy when running the deckgl variant locally, so the file is
    # now resident in this directory rather than referenced via the parent.
    assert (deckgl_root / "data" / "socal_20240726_risk.geojson").exists()
