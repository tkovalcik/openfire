...
def data_proxy(object_name: str):
    ...
    legacy_demo_asset = (STATIC_ROOT.parent / "frontend-socal" / "data" / object_name).resolve()
    if (
        STATIC_ROOT.name == "frontend-socal-deckgl"
        and legacy_demo_asset.exists()
        and legacy_demo_asset.is_file()
        and legacy_demo_asset.parent == (STATIC_ROOT.parent / "frontend-socal" / "data").resolve()
    ):
        return FileResponse(legacy_demo_asset)
